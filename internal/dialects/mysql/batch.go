...
func (t *Target) BatchQuery(ctx context.Context, request core.BatchRequest) (*core.BatchResult, error) {
	...
	for i, query := range request.Queries {
		minimal := *result
		minimal.Results = append(append([]*core.QueryResult(nil), result.Results...), &core.QueryResult{QueryID: fmt.Sprintf("%s/%d", batchID, i+1), Target: t.config.Name, Engine: t.config.Engine, Environment: t.config.Environment, Consistency: t.config.Consistency, Database: t.config.Database})
		if b, _ := json.Marshal(&minimal); len(b) > t.limits.MaxResultBytes {
		...
		remaining := t.limits.MaxResultBytes - len(current)
		if remaining < 1 {
			remaining = 1
		}
