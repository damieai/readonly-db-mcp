...
func metadataOperation(version, name string) error {
	...
	if !op.Implemented || (name != "resolve" && name != "mappings" && name != "field_mappings" && name != "aliases" && name != "settings" && name != "get_script") {
...
func capabilities(version string) map[string]string {
	result := map[string]string{"query_dsl": "implemented_native_visitors", "scripts_runtime_fields": "implemented_painless_expression_lookup", "aggregations": "implemented_native_visitors", "vector_hybrid": "implemented_supplied_vectors_local_fusion", "es_batch": "implemented_independent_and_pit", "inference": "awaiting_authority_profile", "plugins": "stock_distribution_only", "suggest_collate": "implemented_frozen_native_template_scalar_candidates", "sql": "implemented_native_queries_translate_owned_cursors", "esql": "implemented_synchronous_upstream_grammar", "esql_enrich": "requires_explicit_cluster_snapshot_scope", "esql_pragmas": "awaiting_resource_profile", "eql": "implemented_synchronous_upstream_grammar", "api_key_auth": "awaiting_authority_profile", "remote_grants": "validated_readonly_local_queries_only", "cross_cluster": "awaiting_remote_key_and_scope_proof", "qualification": "fixture_tested_not_server_certified"}
